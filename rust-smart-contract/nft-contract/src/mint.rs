...
impl Contract {
    ...
    pub fn nft_mint(
        ...
        assert!(1000 - self.tokens_count > 0, "No NFTs to buy left");
        ...
        assert_eq!(
            self.public_sales || (!self.public_sales && self.og_list.contains(&receiver_id) && tokens_set_len < 5)
                || (!self.public_sales && self.white_list.contains(&receiver_id) && tokens_set_len < 1),
            true,
        ...
        // assert!(ONE_NEAR <= attached_deposit, "Not enough deposit");

        // create a royalty map to store in the token
        let mut royalty = HashMap::new();

        royalty.insert(env::current_account_id(), 1000);
  ...
  pub fn nft_mint_by_id(&mut self, receiver_id: AccountId, token_id: u128){
        //measure the initial storage being used on the contract
        let initial_storage_usage = env::storage_usage();
    ...
    pub fn white_list_get_unused(
        ...
        let mut res: Vec<near_sdk::AccountId> = Vec::with_capacity(self.white_list.len());

        for x in self.white_list.iter().cloned() {
            let mut tokens_set_len = self.tokens_per_owner.get(&x).unwrap_or_else(|| {
                //if the account doesn't have any tokens, we create a new unordered set
                UnorderedSet::new(
                    StorageKey::TokenPerOwnerInner {
                        //we get a new unique prefix for the collection
                        account_id_hash: hash_account_id(&x),
                    }
                        .try_to_vec()
                        .unwrap(),
                )
    ...
    pub fn og_list_get_unused(
        ...
        let mut res: Vec<near_sdk::AccountId> = Vec::with_capacity(self.og_list.len());

        for x in self.og_list.iter().cloned() {
            let mut tokens_set_len = self.tokens_per_owner.get(&x).unwrap_or_else(|| {
                //if the account doesn't have any tokens, we create a new unordered set
                UnorderedSet::new(
                    StorageKey::TokenPerOwnerInner {
